...
class TMTree:
    ...
    def __init__(self, name: str, subtrees: list[TMTree],
        ...
        6
        """
        if name == "" or data_size < 0:
            raise ValueError

        if not subtrees and data_size == 0:
            raise ValueError

        for subtree in subtrees:
            if subtree._parent_tree is not None:
                raise ValueError
    ...
    def is_displayed_tree_leaf(self) -> bool:
        ...
        False
        """
        if self._parent_tree is None and not self._expanded:
            return True
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        if rect[2] > rect[3]:
            ...
            for subtree in self._subtrees:
                proportions[subtree] = subtree.data_size / total

            start_x = rect[0]
            start_y = rect[1]

            for subtree in self._subtrees:
                subtree.update_rectangles(pygame.Rect(start_x, start_y, int(proportions[subtree] * rect[2]), rect[3]))
                start_x += proportions[subtree] * rect[2]
        ...
        if rect[3] > rect[2]:
            ...
            for subtree in self._subtrees:
                subtree.update_rectangles(pygame.Rect(start_x, start_y, rect[2], int(proportions[subtree] * rect[3])))
                start_y += proportions[subtree] * rect[3]
    ...
    def get_rectangles(self) -> list[tuple[tuple[int, int, int, int],
        ...
        for subtree in self._subtrees:
            if subtree.is_displayed_tree_leaf():
                listy.append((subtree.rect, subtree._colour))
