...
def test_no_pay() -> None:
    e = SalariedEmployee(24, 'Kobe Bryant', 24000.0)
    assert e.total_pay() == 0.0


def test_pay_deduction_company() -> None:
    corp = Company([SalariedEmployee(23, 'Lebron James', -4800.0),
                   HourlyEmployee(11, 'DeMar DeRozan', 100.0, 10.0)])
    corp.pay_all(date(2018, 6, 28))
    assert corp.total_payroll() == 600
